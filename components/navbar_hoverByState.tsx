...
const NavItem: FC<NavItemI> = ({ label, to, navbarOnTop, currentHover, setCurrentHover, subMenu }) => {
  const router = useRouter();
  const { asPath } = router;
   ...
   <a
        className={`
          ...
          ${
            asPath === "/robot-as-a-service" &&
            navbarOnTop &&
            styles.menuItem_atRaasPage
          } 
          ${asPath === to && styles.menuActive}
        `}
      >
  */      
  /*
